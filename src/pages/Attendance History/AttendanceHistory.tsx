...
const S: Record<string, React.CSSProperties> = {
  ...
  headerTitle: { margin: "0 0 0.25rem 0", fontSize: "1.75rem", fontWeight: 700 },
  ...
  programCardTitle: { margin: 0, fontSize: "1.1rem", fontWeight: 700, color: "#1e293b" },
  programMeta: {
    display: "flex",
    alignItems: "center",
    gap: "0.4rem",
...
const injectKeyframes = () => {
  if (document.getElementById("ah-keyframes")) return;
...
function AttendanceHistory() {
  ...
  useEffect(() => {
    if (!selectedEventId) { setAttendanceRecords([]); return; }
    setLoadingAttendance(true);
    axios
      .get<AttendanceRecord[]>(`${import.meta.env.VITE_API_URL}/attendance/by-event/${selectedEventId}`)
  ...
  const programsToShow = selectedEvent?.program_id
      ...
      <div style={S.layout}>
        ...
        <main style={{ ...S.main, justifyContent: "center", alignItems: "center" }}>
          <div style={{ textAlign: "center", color: "#94a3b8" }}>
            ...
            <p style={{ marginTop: "0.75rem" }}>Loading attendance history...</p>
          </div>
        </main>
      </div>
    );
  ...
  return (
    <div style={S.layout}>
      ...
      <main style={S.main}>
        ...
        <div style={S.content}>
          {/* ── Controls Bar ── */}
          <div style={S.controlsBar} className="ah-fade-up ah-delay-1">
            {/* Left: event title + subtitle */}
            <div>
              <h2 style={S.eventTitle}>
                <i className="bi bi-calendar-event" style={{ color: "#4f46e5", fontSize: "1.4rem" }} />
                ...
                {selectedEvent ? (
                  <>
                    ...
                    {selectedEvent.program_id && (
                      <span style={{ background: "rgba(79,70,229,0.1)", color: "#4f46e5", borderRadius: 6, padding: "2px 8px", fontSize: "0.78rem", fontWeight: 600, marginLeft: 8 }}>
          ...
          {loadingAttendance && (
            <div style={{ textAlign: "center", padding: "3rem" }}>
              ...
              <p style={{ marginTop: "0.5rem", color: "#94a3b8", fontSize: "0.9rem" }}>Loading attendance...</p>
            </div>
          )}
            ...
            <div className="ah-fade-up ah-delay-2">
              {filteredPrograms.length > 0 ? (
                filteredPrograms.map((program) => {
                  const presentCount = program.students.filter((s) => presentIds.has(s.student_id_no)).length;
                  const totalCount = program.students.length;
                  const pct = totalCount > 0 ? Math.round((presentCount / totalCount) * 100) : 0;
                  const pctColor = pct === 100 ? "#198754" : pct >= 75 ? "#0d6efd" : pct >= 50 ? "#856404" : "#dc3545";
                      ...
                      <div style={S.programCardHeader}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" as const }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <span style={{ fontSize: "0.8rem", fontWeight: 700, color: pctColor }}>{pct}%</span>
                          </div>
                          <div style={S.programMeta}>
                            <i className="bi bi-people-fill" style={{ color: "#94a3b8" }} />
                            <span style={S.presentCount}>{presentCount}</span>
                            <span style={{ color: "#94a3b8" }}>/ {totalCount} present</span>
                          </div>
                        </div>
                      </div>
                        ...
                        <table style={S.table}>
                          <thead>
                            <tr>
                              ...
                              <th style={{ ...S.thCenter, width: 100 }}>Year</th>
                              <th style={{ ...S.thCenter, width: 130 }}>Status</th>
                              <th style={{ ...S.thCenter, width: 160 }}>Time</th>
                            </tr>
                          </thead>
                          <tbody>
                            {program.students.map((student, idx) => {
                              const isPresent = presentIds.has(student.student_id_no);
                              const record = attendanceRecords.find((r) => r.student_id_no === student.student_id_no);
                              ...
                              return (
                                <tr
                                  ...
                                  <td style={{ ...S.td, color: "#94a3b8", fontSize: "0.8rem" }}>{idx + 1}</td>
                                  ...
                                  <td style={S.tdCenter}>
                                    {student.year_level ? (
                                      <span style={{ background: "rgba(100,116,139,0.1)", color: "#475569", border: "1px solid #e2e8f0", padding: "2px 8px", borderRadius: 6, fontSize: "0.78rem", fontWeight: 500 }}>
                                        {student.year_level}
                                      </span>
                                  ...
                                  <td style={S.tdCenter}>
                                    {isPresent
                                      ? <span style={S.badgePresent}>✓ Present</span>
                                      : <span style={S.badgeAbsent}>Absent</span>
                                    }
                                  </td>
                                  <td style={{ ...S.tdCenter, color: "#94a3b8", fontSize: "0.82rem" }}>
                                    {record?.attendance_time
                                      ? new Date(record.attendance_time).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })
                                      : <span style={{ color: "#cbd5e1" }}>—</span>}
                                  </td>
                                </tr>
                              );
                ...
                <div style={S.emptyState}>
                  <i className="bi bi-search" style={{ fontSize: "2.5rem", display: "block", marginBottom: "0.75rem", opacity: 0.35 }} />
                  <p style={{ fontSize: "0.95rem", margin: 0 }}>
                    {selectedEventId ? "No students match your search." : "Select an event to view attendance."}
                  </p>
                </div>
              )}
            </div>
